...
export class MqttRepository {
  ...
  async insertMeasurementsBatch(measurements: MqttMeasurement[]): Promise<void> {
    ...
    try {
      ...
      // Ensure devices exist in device_system_status (required for listModules to work)
      // Extract unique (moduleId, chipId) pairs from the batch
      const devices = new Map<string, { moduleId: string; chipId: string }>()
      measurements.forEach(m => {
        const chipId = m.chipId !== 'UNKNOWN' ? m.chipId : m.moduleId
        const key = `${m.moduleId}:${chipId}`
        if (!devices.has(key)) {
          devices.set(key, { moduleId: m.moduleId, chipId })
        }
      })
